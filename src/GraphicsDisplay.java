    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minX <= 0.0 && maxX >= 0.0) {
            ...
            for (double x = unitInterval; x <= maxX; x += unitInterval) {
                Point2D.Double tickMark = xyToPoint(x * 10, 0); // Shift the tick mark position to the right

                // Draw tick mark
                canvas.draw(new Line2D.Double(tickMark.getX(), tickMark.getY() - 5, tickMark.getX(), tickMark.getY() + 5));

                // Draw label
                Rectangle2D boundsTickLabel = axisFont.getStringBounds(Double.toString(x), context);
                canvas.drawString(Double.toString(x), (float) (tickMark.getX() - boundsTickLabel.getWidth() / 2),
